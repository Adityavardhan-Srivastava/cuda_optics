...
// Linux: nvcc -O3 -shared -Xcompiler -fPIC -arch=all -o engine_cuda.so engine_cuda.cu
// Windows: "C:\Program Files\NVIDIA GPU Computing Toolkit\CUDA\v12.2\bin\nvcc.exe" -shared -o engine_cuda.dll engine_cuda.cu -O3 -arch=all -cudart=static -Xlinker /DEF:exports.def -allow-unsupported-compiler -D_ALLOW_COMPILER_AND_STL_VERSION_MISMATCH
// Kindly change the commands as per your requirements.

#include <cuda_runtime.h>
#include <cmath>
#include <cstddef> // for offsetof
#include <cstdio> // required for printf, stderr
...
// --------------------------------------------------------
// 1. DATA STRUCTURES (Match Python Packing with Padding)
// --------------------------------------------------------

struct Ray {
    // 7 Floats total
    float type_id;      // Byte 0: Initialised with -1 (Flag for screen detection)
    float active;       // Byte 4: 1 = active, 0 = dead
    float wavelength;   // Byte 8
    float x;            // Byte 12
    float y;            // Byte 16  
    float dx;           // Byte 20
    float dy;           // Byte 24
...
__device__ float check_lens_surface(const Ray& r, float cc_x, float cc_y, float radius_abs, 
...
__device__ float check_prism_face(const Ray& r, float p1_x, float p1_y, float p2_x, float p2_y, 
...
__device__ float intersect_parabolic(const Ray& r, const Element& el, float& out_nx, float& out_ny) {
...
__device__ float intersect_flat(const Ray& r, const Element& el, float& out_nx, float& out_ny) {
...
__device__ float intersect_spherical(const Ray& r, const Element& el, float& out_nx, float& out_ny) {
...
__device__ float intersect_slab(const Ray& r, const Element& el, float& out_nx, float& out_ny) {
...
__device__ float intersect_thick_lens(const Ray& r, const Element& el, float& out_nx, float& out_ny) {
...
__device__ float intersect_prism(const Ray& r, const Element& el, float& out_nx, float& out_ny, int& kill_flag) {
...
__device__ float intersect_aperture(const Ray& r, const Element& el, float& out_nx, float& out_ny) {
...
__device__ float get_index(const Element& el, int offset, float wav_nm) {
...
__device__ void apply_refraction(Ray& r, float nx, float ny, float n_glass) {
...
__device__ void interact_parabolic(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_flat_mirror(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_spherical_mirror(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_screen(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_aperture(Ray& r, const Element& el, float nx, float ny) {

    // If we are here, we hit the walls and the ray is absorbed
    r.active = 0.0f;
}

__device__ void interact_slab(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_thick_lens(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_prism(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_thin_lens(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_trans_grating(Ray& r, const Element& el, float nx, float ny) {
...
__device__ void interact_refl_grating(Ray& r, const Element& el, float nx, float ny) {
...
// --------------------------------------------------------
// The __global__ kernal
// --------------------------------------------------------

__global__ void simulate_kernel(Ray* rays, int num_rays, 
                                Element* elements, int num_elements, 
                                int rounds, 
                                float* interaction_ptr, float* record_ptr) {
    
    int i = blockIdx.x * blockDim.x + threadIdx.x; // Standard calculation to find the index
    if (i >= num_rays) return;

    Ray r = rays[i]; // Local copy for register speed

    // Calculate slot in the 1D output array and input the inital position
    ...
    for (int j = 0; j < rounds; j++) {
        ...
        for (int k = 0; k < num_elements; k++) {
            float d = -1.0f;
            int current_kill = 0;
            Element el = elements[k]; // Note: Elements are in global memory (slow), ideally cache into Shared Memory but
                                      // not currently implementing to allow the freedom to input as many elements as required
    ...
    rays[i] = r; // Update final state of ray
}
// --------------------------------------------------------
// Host Function (Called by Python)
// --------------------------------------------------------

extern "C" {
EXPORT void simulate_c(float* raw_ray_ptr, int num_rays, 
                                      float* raw_element_ptr, int num_elements, 
                                      int rounds, 
                                      float* interaction_ptr, float* record_ptr) {

    
    // Initialise arguments for cudaMalloc
    Ray* d_rays = nullptr;
    Element* d_elems = nullptr;
    float* d_inter = nullptr;
    float* d_rec = nullptr;
    int threadsPerBlock = 256;
    int blocksPerGrid = 0;

    size_t size_rays = num_rays * sizeof(Ray);
    size_t size_elems = num_elements * sizeof(Element);
    size_t size_inter = 2 * num_rays * (rounds + 1) * sizeof(float);
    size_t size_rec = num_rays * sizeof(float);

    cudaError_t err;
    const char* error_stage = "Initialization"; // Tracker

    // Allocate memory in the GPU
    error_stage = "Malloc rays";
    err = cudaMalloc(&d_rays, size_rays);
    if (err != cudaSuccess) goto Error;

    error_stage = "Malloc elements";
    err = cudaMalloc(&d_elems, size_elems);
    if (err != cudaSuccess) goto Error;

    error_stage = "Malloc interactions";
    err = cudaMalloc(&d_inter, size_inter);
    if (err != cudaSuccess) goto Error;

    error_stage = "Malloc record";
    err = cudaMalloc(&d_rec, size_rec);
    if (err != cudaSuccess) goto Error;

    // Initialise the output array to an array of all NaN
    error_stage = "Initialization of interaction array";
    err = cudaMemset(d_inter, 0xFF, size_inter); 
    if (err != cudaSuccess) goto Error;

    // Copy data from the Host to Device
    error_stage = "Copying rays from CPU to GPU";
    err = cudaMemcpy(d_rays, raw_ray_ptr, size_rays, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) goto Error;
    
    error_stage = "Copying elements from CPU to GPU";
    err = cudaMemcpy(d_elems, raw_element_ptr, size_elems, cudaMemcpyHostToDevice);
    if (err != cudaSuccess) goto Error;

    // Launch the threads
    blocksPerGrid = (num_rays + threadsPerBlock - 1) / threadsPerBlock;
    
    simulate_kernel<<<blocksPerGrid, threadsPerBlock>>>(d_rays, num_rays, d_elems, num_elements, rounds, d_inter, d_rec);
    
    error_stage = "Kernel launch";
    err = cudaGetLastError();
    if (err != cudaSuccess) goto Error;

    error_stage = "Kernel execution (sync)";
    err = cudaDeviceSynchronize();
    if (err != cudaSuccess) goto Error;

    // Copy back the results
    error_stage = "Copying interactions from GPU to CPU";
    err = cudaMemcpy(interaction_ptr, d_inter, size_inter, cudaMemcpyDeviceToHost);
    if (err != cudaSuccess) goto Error;

    error_stage = "Copying the record array from GPU to CPU";
    err = cudaMemcpy(record_ptr, d_rec, size_rec, cudaMemcpyDeviceToHost);
    if (err != cudaSuccess) goto Error;
                                        
    // Cleanup
    Error:
        if (d_rays) cudaFree(d_rays);
        if (d_elems) cudaFree(d_elems);
        if (d_inter) cudaFree(d_inter);
        if (d_rec) cudaFree(d_rec);
        
        if (err != cudaSuccess) { // Print the error along with where the code failed
            fprintf(stderr, "CUDA Error at '%s': %s (Code %d)\n", 
                    error_stage, cudaGetErrorString(err), (int)err);
        }
        cudaDeviceReset();
}
}
